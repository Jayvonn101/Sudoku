...
// Java reflection — lets us read private fields from Sudoku that we can't access directly
...
public class SudokuTest {
    ...
    // The Sudoku object we'll test — shared across all tests in this class
    ...
    void constructor_validDimensions_zeroedGrid() throws Exception {
        // 'grid' is a private field in Sudoku, so we use reflection to access it
        // getDeclaredField gets a private field by name
        Field gridField = Sudoku.class.getDeclaredField("grid");
        // setAccessible(true) overrides the private modifier so we can read it
        gridField.setAccessible(true);
        // .get(sudoku) retrieves the value of that field from our sudoku instance
        int[][] grid = (int[][]) gridField.get(sudoku);
    ...
    void constructor_nonSquareDimensions_throwsException() {
        // assertThrows checks that creating a Sudoku with unequal rows/cols
    ...
    void constructor_zeroDimensions_throwsException() {
        ...
        assertThrows(IllegalArgumentException.class, () -> new Sudoku(-1, -1));
    }
    ...
    void isValid_validPlacement_returnsTrue() {
        ...
        assertTrue(sudoku.placeNum(0, 0, 5));
    }
    ...
    void isValid_duplicateInColumn_returnsFalse() {
        ...
        assertFalse(sudoku.placeNum(8, 0, 5));
    }
    ...
    void isValid_duplicateInBox_returnsFalse() {
        ...
        assertFalse(sudoku.placeNum(2, 2, 5));
    }
    ...
    void isValid_cornerCells_worksCorrectly() {
        ...
        assertTrue(sudoku.placeNum(8, 8, 2));
    }
    ...
    void isValid_middleBox_worksCorrectly() {
        ...
        assertTrue(sudoku.placeNum(4, 4, 7));
        // [3][5] is also in the middle box, so placing 7 there should fail
        assertFalse(sudoku.placeNum(3, 5, 7));
    }
    ...
    void placeNum_outOfBoundsRow_rejected() {
        ...
        assertFalse(sudoku.placeNum(9, 0, 1));
    }
    ...
    void placeNum_outOfBoundsColumn_rejected() {
        ...
        assertFalse(sudoku.placeNum(0, -1, 1));
    ...
    void placeNum_numberBelowOne_rejected() {
        ...
        assertFalse(sudoku.placeNum(0, 0, -1));
    }
    ...
    void solve_alreadySolved_remainsUnchanged() throws Exception {
        ...
        int[][] before = deepCopy((int[][]) gridField.get(sudoku));

        assertTrue(sudoku.solve());

        // The grid after solve should be identical to the snapshot
        int[][] after = (int[][]) gridField.get(sudoku);
